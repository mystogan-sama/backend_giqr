...
class List(Resource):
    ...
    def get(self):
        ...
        # Assuming you have a SQLAlchemy model named DAFTUNIT_GIS
        # and a session named db

        # Your corrected SQL query with IN clause
        sql_query = """
            SELECT id, RTRIM(UNITKEY) AS UNITKEY, KDUNIT, NMUNIT, KDLEVEL, TYPE, AKROUNIT, ALAMAT, TELEPON, STAKTIF, DATECREATE
            FROM DAFTUNIT WHERE KDLEVEL IN (3, 4)
        """
    ...
    def get(self, id):
        ...
        try:
            sqlQuery = (
                ...
                f"FROM DAFTUNIT_GIS AS l "
                f"LEFT JOIN PEGAWAI AS p ON l.UNITKEY = p.UNITKEY "
                f"WHERE p.UNITKEY = (SELECT TOP 1 l.UNITKEY FROM DAFTUNIT_GIS l WHERE l.id = {id}) AND p.KDPOSTTD = '1';"
                ...
                f"FROM DAFTUNIT_GIS l LEFT OUTER JOIN SensusTanah s "
                f"ON l.ID = s.id_DAFTUNIT_GIS "
                f"WHERE l.ID = {id}"
            )
            ...
            if result:
                ...
                resultFinal['id'] = result[0]['id_DAFTUNIT_GIS']
