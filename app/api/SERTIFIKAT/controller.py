...
class List(Resource):
    ...
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument('getSertifikat', type=str, required=True, help='Your help message for getDaftAset')
        ...
        if args['getSertifikat'] == '1' and args['UNITKEY']:
            sql_query = """EXEC GET_ASET_FOR_ADDSERT @UNITKEY=:unitkey"""

            # Menjalankan query dengan parameter yang sesuai
            result = db.session.execute(sql_query,{'unitkey': args['UNITKEY']})
