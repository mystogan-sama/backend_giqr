...
class List(Resource):
    ...
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument('getDaftBarang', type=str, required=True, help='Your help message for getDaftAset')
        ...
        if args.get("getDaftBarang") == '1' and args['ASETKEY'] and args['UNITKEY']:
            # Your SQL query with ROW_NUMBER
            sql_query = """EXEC GET_DAFTBRG_FORQR @ASETKEY=:asetkey, @UNITKEY=:unitkey"""

            # Execute the query with parameters
            result = db.session.execute(sql_query, {'asetkey': args['ASETKEY'], 'unitkey': args['UNITKEY']})
    ...
    def get(self, id):
        ...
        try:
            sqlQuery = (
                ...
                f"WHERE p.UNITKEY = (SELECT TOP 1 l.UNITKEY FROM QR_ASET l WHERE l.id = {id}) AND p.KDPOSTTD = '1';"
                ...
                f"l.ID as id_QR_ASET, l.[IDBRG], l.[UNITKEY], l.[ASETKEY], l.[KDUNIT], l.[NMUNIT], "
                ...
                f"FROM QR_ASET l LEFT OUTER JOIN SensusTanah s "
                f"ON l.ID = s.id_QR_ASET "
                f"WHERE l.ID = {id}"
            )
            ...
            if result:
                ...
                resultFinal['id'] = result[0]['id_QR_ASET']
